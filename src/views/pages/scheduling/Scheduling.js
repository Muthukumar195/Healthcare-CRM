...
import { getPatient, getAppointments } from "../../../redux/actions/patientActions";
import { getDoctorAvailability } from "../../../redux/actions/doctorActions";
            ...
            <ButtonGroup>
              <button
                ...
                onClick={() => {
                  ...
                  this.props.getAppointments(moment(this.props.date).toDate(), "dayCalendar")
                }}
              >
                Day
              </button>
              <button
                ...
                onClick={() => {
                  this.props.getAppointments(moment(this.props.date).toDate(), "week")
    ...
    _.forEach(this.props.appointments, (appoint) => {
      for (var c = 1; c <= (appoint.length / 10); c++) {
        ...
        if (startApointTime.isBetween(startTime, endTime)) {
          if (!_.isEmpty(appoint.patient)) {
            match = {
              data: appoint,
              status: appoint.status,
              name: _.toUpper(`${appoint.patient[0].firstName} ${appoint.patient[0].lastName}`)
            }
          }
                          ...
                          <td key={r} className={`timeScheduled cursor-pointer ${this.getAppointmentClassName(appoint.status)} `} onClick={
                            () => {
                              if (_.includes(appoint.status, "Waiting")) {
                                this.props.parentProps.history.push(`/patient-preview/${appoint.data.patient[0]._id}/${appoint.data._id}`)
                              }
...
class CalendarApp extends React.Component {
  ...
  static getDerivedStateFromProps(props, state) {
    if (props.app.sidebar !== state.sidebar ||
      ...
      props.availabilities.data != state.availabilities
    ) {
      var appointments = []
      ...
      if (!_.isEmpty(appointments)) {
        weekAppointments = appointments.list.map((event) => {
          event.title = _.toUpper(`${event.patient[0].firstName} ${event.patient[0].lastName}`);
      ...
      var availabilities = [];
      if ((!_.isEmpty(props.availabilities))) {
        availabilities.push(props.availabilities.data)
      }
      return {
  ...
  constructor(props) {
    ...
    props.getPatient();
    props.getAppointments(moment().toDate(), this.state.selectedView);
    props.getDoctorAvailability(moment().toDate(), this.state.selectedView);
  }

  componentDidMount() {
    this.interval = setInterval(() => {
      this.props.getAppointments(moment(this.state.navigationDate).toDate(), this.state.selectedView);
    }, 5000);
  ...
  render() {
    ...
    const { events, views, sidebar, appointments, appointmentCount, loader, availabilities } = this.state;
      ...
      <div className="app-calendar position-relative">
            ...
            <Calendar
              ...
              onNavigate={(date, view) => {
                ...
                if (view == "dayCalendar") {
                  ...
                  this.props.getAppointments(moment(date).toDate(), view)
                } else if (view == "week") {
                  console.log("onNavigate week")
                  this.props.getAppointments(moment(date).startOf('isoWeek').toDate(), "week")
                }
              }}
              startAccessor="start"
              endAccessor="end"
              ...
              timeslots={1}
              components={{
                toolbar: props => (<Toolbar {...props} getAppointments={(date, view) => {
                  this.setState({ navigationDate: date, selectedView: view, loader: true })
                  console.log("toolbar")
                  this.props.getAppointments(date, view)
                }}
        ...
        <AddEventSidebar
          ...
          closeForm={() => {
            ...
            this.props.handleSidebar(false)
            this.props.getAppointments(this.state.navigationDate, this.state.selectedView)
            this.state.appTime = ''
...
const mapStateToProps = (state) => {
  ...
  const { patient, auth, doctor } = state;
  return {
    app: state.calendar,
